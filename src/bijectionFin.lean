...
lemma disjoint_union_preserves_fin (s1 s2: set A) [has_mem ↥(s1 ∪ s2) (set A)] [∀ (a:A) (s:set A), decidable (a ∈ s)] (s1_fin: is_finite s1) (s2_fin: is_finite s2) (disj: disjoint s1 s2): is_finite (s1 ∪ s2) :=
begin
  unfold is_finite,
  unfold is_finite at s1_fin,
  ...
  existsi (n_s1 + n_s2),
  existsi ( λ (a: s1 ∪ s2), if a ∈ s1 then f_s1 a else f_s2 a),
  sorry,
end
