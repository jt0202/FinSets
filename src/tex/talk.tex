...
\begin{frame}{Types as Space}
    \begin{tikzpicture}[
        node distance=1.5cm,
        every node/.style={draw, align=center, font=\scriptsize, inner sep=1pt}
    ]
      \node (a1) {$\{a\} \cup (\{b\} \cup \{c\})$};
      \node [above=of a1] (a2) {$(\{a\} \cup \{b\}) \cup \{c\}$};
      \node [right=of a2] (a3) {$(\{b\} \cup \{a\}) \cup \{c\}$};
      \node [below=of a3] (b1) {$\{d\} \cup \{d\}$};
      \node [below=of b1] (b2) {$\{d\}$};
